  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await axios.get(
        `${apiPath.prodPath}/api/users/user/?${filterBy}=${searchTerm}`
      );
      setAllEmp(res.data);
    } catch (err) {
      console.error(err);
    ...
    <main className="flex flex-col">
      <div className="flex w-full flex-row flex-wrap justify-between ">
        <div className="w-full flex flex-row gap-2 mb-[24px] h-[34px]">
          <Image src="/employeesLarge.png" alt="employee" width={40} height={31.7} priority/>
          <h1 className="font-satoshi font-semibold text-2xl ml-[20px]">Employees</h1>
        </div>
        ...
        <form onSubmit={handleSearch} className="flex flex-row gap-4 w-full items-center">
                ...
                className="rounded-full text-gray-800 h-[37px] w-[300px] "
              /> */}
                 <select
                    value={filterBy}
