...
import ProductServiceTable from "../subcomponents/tables/purchaseSaleTable";
import AddPurchaseSalelist from "../subcomponents/drawers/purchaseSaleList";
import { ClearAll } from "@mui/icons-material";


function PurchaseComp({ picklistName }) {
  ...
  function filterOptions() {
    ...
    if(picklistName == "Products & Services"){
       sorts = ["purchaseCategory", "purchaseName", "purchaseTools", "cost", "sellingPrice", "purchaseDescription"];
  ...
  const handleSearch = async (e) => {
    ...
    if (picklistName == "Products & Services") {
      url = `${apiPath.prodPath}/api/purchaseService/getPurchaseSale?${filterBy}=${searchTerm}`;
    }
  ...
  const fetchData = async (page=1)=>{
    ...
    var url = "";
    if (picklistName == "Products & Services") {
      url = `${apiPath.prodPath}/api/purchaseService/getAllPurchaseSale?page=${page}&limit=8`;
    }
    ...
    await axios
      ...
      .then((res) => {
        if (picklistName == "Products & Services"){
          ...
          // if( res.data.purchaseServices.length === 0){
          //   Swal.fire({
          //     icon: "info",
          //     text: "No data found",
          //   });
  ...
  const refreshData = async (page=1) => {
    ...
    await axios
      .get(url)
      .then((res) => {if (picklistName == "Products & Services"){
        setPicklistData(res.data);
      }else{
        console.log("No data found");
      }
  ...
  const addPicklist = async (data) => {
    var url = "";
    if (picklistName == "Products & Services") {
      url = `${apiPath.prodPath}/api/purchaseService/addPurchaseSale`;
    }
  ...
  const handleTest=(data)=>{
   ...
   const handleClearSearch = (page = 1) => {
    setSearchTerm("");
    setFilterBy("");
    setLoader(true);
    axios
      .get(`${apiPath.prodPath}/api/purchaseService/getAllPurchaseSale?page=${page}&limit=8`)
      .then((res) => {
        setPicklistData(res.data);
        setLoader(false);
      })
