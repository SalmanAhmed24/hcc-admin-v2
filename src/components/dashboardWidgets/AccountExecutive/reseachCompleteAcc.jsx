...
function ResearchCompleteClient( {clientItem}) {
  ...
  setLoader(true);
  try {
    const res = await axios.get(`${apiPath.prodPath}/api/clients/clientResearchComplete`, {
    params: {
      status: "Lead",
  ...
  setLoader(true);
  try {
     axios.get(`${apiPath.prodPath}/api/clients/clientResearchComplete`, {
    params: {
      status: "Lead",
      researchStatus: "completed",
      assignedTo: `${user.user.firstName} ${user.user.secondName}`,
        ...
        {loader ? "loading...." : (
          <div className="flex flex-col gap-2 border-2 border-[#452C95] shadow-lg h-[100%] rounded-[12px] p-4 bg-[#231C46] w-[100%]">
            <div className="flex flex-row items-center gap-2">
                  <p className="w-1/3">Client Research Completed</p>
            <p className={`w-1/3 ${clientColor}`}>{newClients.length > 0 ? `Research complete for ${newClients.length} new clients.` : "No new clients research completed."}</p>
              <Button
              ...
              <div className="flex justify-start mb-4">
                <Accordion className="w-full">
                  <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                    <Typography>Click to View Clients with Completed Research </Typography>
                      </AccordionSummary>
                        <AccordionDetails>
...
export default ResearchCompleteClient;
