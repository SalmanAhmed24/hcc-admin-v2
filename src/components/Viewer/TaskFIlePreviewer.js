...
const TaskFilePreviewDrawer = ({ open, handleClose, file, i, item, previewOpen }) => {
const [previewContent, setPreviewContent] = useState(null);
  ...
  const handleDownload = async () => {
    try {
      const response = await axios.get(
        `${apiPath.prodPath}/api/tasks/getFile/${item._id}&&${i._id}&&${file._id}`,
        { responseType: "blob" } 
      );
  ...
  const handleSignedUrl = async () => {
      ...
      try {
        const response = await axios.get(
          `${apiPath.prodPath}/api/tasks/getFileUrl/${item._id}&&${i._id}&&${file._id}`
        );
...
export default TaskFilePreviewDrawer;