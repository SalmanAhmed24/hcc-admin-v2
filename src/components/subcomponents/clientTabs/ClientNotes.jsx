...
const ClientNotes = ({item, open}) => {
  ...
  const handleUpload = async (e) => {
    ...
    if (editNote) {
      ...
      await axios
        .patch(`${apiPath.prodPath}/api/clients/editClientNotes/${item._id}&&${noteId}`, formData)
        .then((res) => {
          Swal.fire({
            icon: "success",
      ...
      await axios
        .patch(`${apiPath.prodPath}/api/clients/addClientNote/${item._id}`, formData)
        .then((res) => {
          Swal.fire({
            icon: "success",
  ...
  const handleDelete = async (id) => {
    Swal.fire({
          ...
          if(result.isConfirmed){
            await axios
              .patch(`${apiPath.prodPath}/api/clients/deleteClientNote/${item._id}&&${id}`)
              .then((res) => {
                Swal.fire({
                  icon: "success",
  ...
  return (
      ...
      <Table className="bg-[#231C46] rounded-[12px] font-satoshi">
        ...
        <TableBody>
          { clientItem.clientNotes !== undefined && clientItem.clientNotes.map((i) => 
