...
import TaskFilePreviewDrawer from "@/components/Viewer/TaskFIlePreviewer";



const TaskFile = ({ open, item }) => {
  ...
  const refreshData = async () => {
    ...
    setOldFiles([]);
    await axios
      .get(`${apiPath.prodPath}/api/tasks/${item._id}`)
      .then((res) => {
        setClientItem(res.data);
        console.log(res.data.attachments)
        setAttachments(res.data.attachments);
  ...
  const handleUpload = async (e) => {
    ...
    if (editAttachments){
      ...
      console.log(fileCategory.value, " ", note, " ", clientItem.taskTitle, " ", createdBy.value);
      ...
      formData.append("user", clientItem.taskTitle);
  ...
  async function sendFiles(data) {
    ...
    try {
      const res = await axios.patch(
        `${apiPath.prodPath}/api/tasks/addFiles/${clientItem._id}`,
        data,
  ...
  async function editFiles(data) {
    ...
    try {
      const res = await axios.patch(
        `${apiPath.prodPath}/api/tasks/editFile/${client._id}&&${attachmentId}&&${fileId}`,
        data,
        { headers: { "Content-Type": "multipart/form-data" } }
      );
  ...
  const handleDelete = async (i, obj) => {
        ...
        try {
          ...
          await axios.patch(
            `${apiPath.prodPath}/api/tasks/delFile/${item._id}&&${i._id}&&${obj._id}`, {oldFiles}
          ).then(async (res) => {
                  console.log(res);
                  Swal.fire({
                    icon: "success",
      ...
      <Table className="bg-[#231C46] rounded-[12px] font-satoshi">
        ...
        <TableHeader>
          <TableRow className="w-fit, h-[58px] font-satoshi text-lg text-[#E1C9FF]">
            ...
            <TableHead className="text-[#E1C9FF]" style={{ minWidth: 100 }}>Task</TableHead>
